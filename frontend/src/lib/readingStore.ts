...
export const buildDocKey = (file: File) => `${file.name}|${file.size}|${file.lastModified}`;

export const getDocument = async (docKey: string) =>
	runTransaction<DocumentRecord | undefined>(STORE_DOCUMENTS, "readonly", (tx) =>
		requestToPromise(tx.objectStore(STORE_DOCUMENTS).get(docKey) as IDBRequest<DocumentRecord>),
	);
...
export const updateDocument = async (docKey: string, patch: Partial<DocumentRecord>) => {
	await runTransaction(STORE_DOCUMENTS, "readwrite", async (tx) => {
		const store = tx.objectStore(STORE_DOCUMENTS);
		const existing = (await requestToPromise(store.get(docKey) as IDBRequest<DocumentRecord>)) as
			| DocumentRecord
			| undefined;
		if (!existing) return;
...
export const getRecentDocuments = async (limit = 12) =>
	runTransaction<DocumentRecord[]>(STORE_DOCUMENTS, "readonly", async (tx) => {
		const store = tx.objectStore(STORE_DOCUMENTS);
		const all = (await requestToPromise(store.getAll() as IDBRequest<DocumentRecord[]>)) as DocumentRecord[];
...
export const listSessions = async (docKey: string) =>
	runTransaction<SessionRecord[]>(STORE_SESSIONS, "readonly", async (tx) => {
		...
		const items = (await requestToPromise(index.getAll(docKey) as IDBRequest<SessionRecord[]>)) as SessionRecord[];
...
export const getSession = async (sessionId: string) =>
	runTransaction<SessionRecord | undefined>(STORE_SESSIONS, "readonly", (tx) =>
		requestToPromise(tx.objectStore(STORE_SESSIONS).get(sessionId) as IDBRequest<SessionRecord>),
	);

export const touchSession = async (sessionId: string) => {
	await runTransaction(STORE_SESSIONS, "readwrite", async (tx) => {
		const store = tx.objectStore(STORE_SESSIONS);
		const existing = (await requestToPromise(store.get(sessionId) as IDBRequest<SessionRecord>)) as
			| SessionRecord
			| undefined;
		if (!existing) return;
...
export const updateSessionTitle = async (sessionId: string, title: string | null) => {
	await runTransaction(STORE_SESSIONS, "readwrite", async (tx) => {
		const store = tx.objectStore(STORE_SESSIONS);
		const existing = (await requestToPromise(store.get(sessionId) as IDBRequest<SessionRecord>)) as
			| SessionRecord
...
export const clearSessionMessages = async (sessionId: string) => {
	await runTransaction(STORE_MESSAGES, "readwrite", async (tx) => {
		...
		const keys = (await requestToPromise(index.getAllKeys(sessionId) as IDBRequest<IDBValidKey[]>)) as IDBValidKey[];
		keys.forEach((key) => {
			store.delete(key);
		});
	});
...
export const loadMessages = async (sessionId: string) =>
	runTransaction<StoredMessage[]>(STORE_MESSAGES, "readonly", async (tx) => {
		const store = tx.objectStore(STORE_MESSAGES);
		const index = store.index("by_sessionId");
		const items = (await requestToPromise(index.getAll(sessionId) as IDBRequest<StoredMessage[]>)) as StoredMessage[];
